...
## [2.3.0] - Mon Aug 10 11:46:16 JST 2026
...
- **修正**: 「最終更新」列がスキャンごとに更新されず初回発見時刻のまま固定されていた問題を修正
- **整理**: 参照ゼロの `Views/ScanCycleDialog.xaml(.cs)` を削除。`ShowAbout` のバージョン文字列ハードコードを `CurrentVersion` 定数参照に統一

## [2.2.1] - Wed Jun 03 11:42:28 JST 2026

- **UI 修正**: ワーカーインジケーターの停止状態ラベルを「ワーカー停止」→「ワーカー停止中」に変更（停止操作ボタンと誤認されにくくするため）

## [2.2.0] - Sat Apr 25 07:42:46 JST 2026

- **新機能**: リスト「参加 PC」列を追加。`({ProjectName}_00_Logs)/machines.htm` を解析し、レンダリングに参加している PC 名を表示する

## [2.1.1] - Sat Apr 25 07:42:46 JST 2026

- **UI 改善**: 「▶ 参加」→「▶ ワーカー起動」、「■ 参加中」→「■ ワーカー停止」に変更（AWS Deadline スタイル）
- **UI 改善**: ステータスバーの「○ 未参加/◎ 参加中」→「○ ワーカー停止/◎ ワーカー稼働中」に変更
- **UI 改善**: 監視フォルダ未設定時は「▶ 監視」ボタンを無効化（`CanStartMonitoring` バインド）
- **新機能**: 操作メニューに「監視フォルダを開く」を追加
- **UI 改善**: 「移動先フォルダへ移動」コンテキストメニュー項目を移動先未設定時に無効化
- **UI 改善**: ステータスバーにタスク件数を表示
- **UI 改善**: タスクリストが空の場合にヒントテキストを表示
- **UI 改善**: ウィンドウに `MinWidth=600 / MinHeight=400` を設定（縮小しすぎ防止）

## [2.1.0] - Sat Apr 18 19:06:22 JST 2026

- **UI 刷新**: ツールバーを 6 ボタンから 3 ボタンに整理
  - 「監視開始」「監視停止」→「▶ 監視」トグルボタン（監視中は「■ 監視中」+ 緑色）
  - 「参加開始」「参加停止」→「▶ 参加」トグルボタン（参加中は「■ 参加中」+ オレンジ色）
  - 「SCAN NOW (F5)」→「SCAN NOW」（F5 キーバインドは継続）
- **設定ダイアログ**: 散在していた設定をセットアップ > 設定... に一元化
  - 監視フォルダ・移動先フォルダ・スキャン間隔
  - aerender フォールバックパス（自動検出できない場合のみ使用）の説明を追記
  - aerender ウィンドウを残す チェックボックス
- **メニュー簡素化**: セットアップメニューを「設定...」1項目のみに整理
- 監視/参加フォルダ未設定時はトグル操作時にメッセージで「設定... から設定してください」と案内

## [2.0.3] - Sat Apr 18 19:06:22 JST 2026

- **新機能**: aerender ウィンドウの動作をユーザーが選択できるように
  - セットアップメニューに「aerender ウィンドウを残す」チェック項目を追加
  - デフォルト OFF（`/C`）: 完了後にウィンドウが自動で閉じる
  - ON（`/K`）: 完了後もウィンドウが残り出力を確認できる
  - 参加モード・手動「aerenderで参加」の両方に適用
  - 常に最小化起動（タスクバーに収まり作業の邪魔にならない）
